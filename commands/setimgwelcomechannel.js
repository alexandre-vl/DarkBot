exports.run = (client, message, droit) => {
    ...
    if(!channel){
        const droit = new Discord.MessageEmbed()
            .setColor(client.config.red)
            .setDescription(`<a:uncheckmoove:740634696198914070> **Veulliez préciser un channel (mention)**`);
        return message.channel.send(droit);
    }
    srv.set(`${message.guild.id}`, channel, "imgwelcchannel");
    srv.set(`${message.guild.id}`, "on", "statimgwelc");
