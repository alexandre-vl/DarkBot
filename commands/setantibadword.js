exports.run = (client, message, args, droit) => {
    ...
    if(args[0] === "on"){
        srv.set(`${message.guild.id}`, "on", "statbadword");
        const droit = new Discord.MessageEmbed()
            .setColor(client.config.green)
            .setDescription("<a:checkmarkmoove:740634693078089730> L'anti badword est ON");
        message.channel.send(droit);
    }else if(args[0] === "off"){
        srv.set(`${message.guild.id}`, "off", "statbadword");
        const droit = new Discord.MessageEmbed()
            .setColor(client.config.green)
            .setDescription("<a:checkmarkmoove:740634693078089730> L'anti badword est OFF");
        message.channel.send(droit);
    }else {
        const droit = new Discord.MessageEmbed()
            .setColor(client.config.red)
